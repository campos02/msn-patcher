...
!define MUI_FINISHPAGE_RUN_TEXT "Launch Yahoo! Messenger 5.5"
...
Section "install"
	...
	File MAINDIR\pcre.dll
	File MAINDIR\XMLParse.dll
	File MAINDIR\Xmltok.dll
	...
	WriteRegStr HKCU ${{YPAGER_KEY}} "IPLookup" "66.218.71.63,64.58.77.85"
	...
	WriteRegStr HKLM "${{ESCARGOT_UNINSTALL_ENTRY}}" DisplayIcon "$INSTDIR\ypager.exe,-0"
	WriteRegStr HKLM "${{ESCARGOT_UNINSTALL_ENTRY}}" DisplayName "Yahoo! Messenger"
	WriteRegStr HKLM "${{ESCARGOT_UNINSTALL_ENTRY}}" Publisher "Yahoo! Inc."
SectionEnd
...
Section "Uninstall"
	...
	Delete "${{YMSGR_FOLDER}}\pcre.dll"
	Delete "${{YMSGR_FOLDER}}\XMLParse.dll"
	Delete "${{YMSGR_FOLDER}}\Xmltok.dll"
	...
	Delete "${{YMSGR_FOLDER}}\kdu_v32R.dll"
